...
# Функция для подключения и создания таблицы, если она не существует
def create_table():
    try:
        conn = psycopg2.connect(dbname='lab10', user='postgres', password='nurym', host='localhost', port='5432')
        cur = conn.cursor()
        create_query = '''
...
def insert_score(name, score, level):
    try:
        ...
        print(f"Error while inserting score: {e}")
...
def get_scores(name):
    try:
        ...
        print(f"Error while fetching scores: {e}")
        return []
